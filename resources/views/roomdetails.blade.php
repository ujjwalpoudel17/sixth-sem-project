    ...
    <div class="grid grid-cols-1 mb-5 mt-5">
        <div class="grid grid-cols-2  mt-10 gap-20 mx-32 border-b-2 rounded-1">
            ...
            <div class="">
                ...
                <img src="{{asset('images/room/'.$roommanagement->photopath)}}"
                     class="rounded-2xl w-full h-64 object-fill " 
                     alt="Hotel Moonlight and Bar" />
                     
                
                <div class="grid grid-cols-3 gap-4 mt-4">
                    <img src="{{asset('images/room/'.$roommanagement->photopath2)}}" class="w-full h-44 object-cover rounded-lg shadow-lg" alt="" />
                    <img src="{{asset('images/room/'.$roommanagement->photopath3)}}" class="w-full h-44 object-cover rounded-lg shadow-lg" alt="" />
                    <img src="{{asset('images/room/'.$roommanagement->photopath4)}}" class="w-full h-44 object-cover rounded-lg shadow-lg" alt="" />
                </div>
            </div>

           
            <div class="flex flex-col justify-center">
                <p class="mx-10 font-bold text-3xl mb-4 text-center text-blue-500">{{$roommanagement->roomtype}}</p>
                <span class="font-semibold text-blue-500 text-xl">About :</span>   
                <p class="font-light mt-2 mb-4"> 
                {{$roommanagement->about}}
                </p>

                <span class="font-semibold text-blue-500 text-xl">Features :</span>   
                <p class="font-light mt-2">{{$roommanagement->features}}</p>
                <p class="font-light">{{$roommanagement->features}}</p>
                <p class="font-light mb-4">{{$roommanagement->features}}</p>

                <span class="font-semibold text-blue-500 text-xl">Available Facilities :</span>   
                <p class="font-light mt-2">{{$roommanagement->facilities}}</p>
                <p class="font-light">{{$roommanagement->facilities}}</p>
                <p class="font-light mb-10">{{$roommanagement->facilities}}</p>
...
<div>
  ...
  <div class="mt-6 border-t border-gray-100 mx-32 border-2">
    <dl class="divide-y divide-gray-100">
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        <dt class="text-sm/6 font-medium text-gray-900 text-center">Checkin:</dt>
        <dd class="mt-1 text-sm/6 text-gray-700 sm:col-span-2 sm:mt-0">{{$roommanagement->rules}}</dd>
      </div>
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        <dt class="text-sm/6 font-medium text-gray-900 text-center">Checkout:</dt>
        <dd class="mt-1 text-sm/6 text-gray-700 sm:col-span-2 sm:mt-0">{{$roommanagement->rules}}</dd>
      </div>
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
  ...
  <div class="px-4 sm:px-0 mx-32 mt-8">
    ...
    <p class="mt-1 max-w-2xl text-gray-500 mb-8">Book it now or never</p>
            ...
            <a href="{{route('bookingform')}}" class=" font-bold bg-blue-500 p-4 rounded-2 text-white w-52 mx-32">I'll Book/Reserve
        </a>
</div>
...
<div>
      ...
      <a href="{{route('roomdetails',$room->id)}}" class="">
        <div class="shadow border-2 rounded-2xl">
          ...
          <p class="mx-4 mt-4 font-bold">{{$room->roomtype}}</p>
          ...
          <p class="text-right mt-8 mx-4 mb-4"><span class="font-medium">Starting Price:</span> {{$room->price}}</p>
        </div>
      </a>
@endforeach
      </div>
